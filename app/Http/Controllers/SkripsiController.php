        ...
        return DataTables::of($proTA)
            ...
            ->addColumn('nim', function ($proTA) {
                return '<p style="margin: 0; font-size: 13px; font-weight: 500;" class="text-muted">' . ucwords($proTA->mahasiswa->nama) . ' (' . $proTA->created_at->format('Y') . ')</p>
                 <p style="margin: 2px 0; font-size: 14px; font-weight: 700;" class="text-dark">' . ucwords($proTA->pendadaran->judul_ta) . '</p>
                 <p style="margin: 0; font-size: 13px; font-weight: 500;" class="text-muted">' . ucwords($proTA->dosen_satu->nama) . '  |  ' . ucwords($proTA->dosen_dua->nama) . '</p>';
            })
            ->rawColumns(['nim'])
            ->make(true);
    }
